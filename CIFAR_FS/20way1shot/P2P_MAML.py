...
def train(args):
    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            file = open('results/P2P_acc1.txt', 'a')
            file.write("%s\n" % float(acc1))
            file.close()
            file = open('results/P2P_acc2.txt', 'a')
            file.write("%s\n" % float(acc2))
            file.close()
            file = open('results/P2P_acc3.txt', 'a')
            file.write("%s\n" % float(acc3))
            file.close()
            file = open('results/P2P_acc4.txt', 'a')
            file.write("%s\n" % float(acc4))
            file.close()

            # Save the test loss results in .txt files
            file = open('results/P2P_loss1.txt', 'a')
            file.write("%s\n" % float(outer_loss1))
            file.close()
            file = open('results/P2P_loss2.txt', 'a')
            file.write("%s\n" % float(outer_loss2))
            file.close()
            file = open('results/P2P_loss3.txt', 'a')
            file.write("%s\n" % float(outer_loss3))
            file.close()
            file = open('results/P2P_loss4.txt', 'a')
            file.write("%s\n" % float(outer_loss4))
            file.close()
