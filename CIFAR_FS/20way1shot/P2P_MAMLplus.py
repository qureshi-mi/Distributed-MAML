...
from model_fs import ConvolutionalNeuralNetwork
...
def split_data(data, labels):
    data1 = torch.zeros((len(data),len(data[0])//4, 3, 32, 32))
    data2 = torch.zeros((len(data),len(data[0])//4, 3, 32, 32))
    data3 = torch.zeros((len(data),len(data[0])//4, 3, 32, 32))
    data4 = torch.zeros((len(data),len(data[0])//4, 3, 32, 32))
    for ind in range(len(data)):
        l1 = torch.where((labels[ind,:] < 5) & (labels[ind,:] >= 0))[0]
        l2 = torch.where((labels[ind,:] < 10) & (labels[ind,:] >= 5))[0]
        l3 = torch.where((labels[ind,:] < 15) & (labels[ind,:] >= 10))[0]
        l4 = torch.where((labels[ind,:] < 20) & (labels[ind,:] >= 15))[0]
...
def train(args):
    ...
    # Initializations
    lr = args.step_size
    dataset = cifar_fs(args.folder,
                       shots=args.num_shots,
                       ways=args.num_ways,
                       shuffle=False,
                       test_shots=args.num_testshots,
                       meta_train=True,
    ...
    model1 = ConvolutionalNeuralNetwork(3,
                                       args.num_ways,
                                       hidden_size=args.hidden_size)
    model2 = ConvolutionalNeuralNetwork(3,
                                       args.num_ways,
                                       hidden_size=args.hidden_size)
    model3 = ConvolutionalNeuralNetwork(3,
                                       args.num_ways,
                                       hidden_size=args.hidden_size)
    model4 = ConvolutionalNeuralNetwork(3,
                                       args.num_ways,
                                       hidden_size=args.hidden_size)
    ...
    for task_idx, (train_input1, train_target1, test_input1, test_target1, train_input2, train_target2, test_input2, test_target2, train_input3, train_target3, test_input3, test_target3, train_input4, train_target4, test_input4, test_target4) in enumerate(zip(train_inputs1, train_targets1, test_inputs1, test_targets1, train_inputs2, train_targets2, test_inputs2, test_targets2, train_inputs3, train_targets3, test_inputs3, test_targets3, train_inputs4, train_targets4, test_inputs4, test_targets4)):
    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            file = open('results/P2Pplus_acc1.txt', 'a')
            file.write("%s\n" % float(acc1))
            file.close()
            file = open('results/P2Pplus_acc2.txt', 'a')
            file.write("%s\n" % float(acc2))
            file.close()
            file = open('results/P2Pplus_acc3.txt', 'a')
            file.write("%s\n" % float(acc3))
            file.close()
            file = open('results/P2Pplus_acc4.txt', 'a')
            file.write("%s\n" % float(acc4))
            file.close()

            # Save the test loss results in .txt files
            file = open('results/P2Pplus_loss1.txt', 'a')
            file.write("%s\n" % float(outer_loss1))
            file.close()
            file = open('results/P2Pplus_loss2.txt', 'a')
            file.write("%s\n" % float(outer_loss2))
            file.close()
            file = open('results/P2Pplus_loss3.txt', 'a')
            file.write("%s\n" % float(outer_loss3))
            file.close()
            file = open('results/P2Pplus_loss4.txt', 'a')
            file.write("%s\n" % float(outer_loss4))
            file.close()
            ...
            with torch.no_grad():
                for param1, param2, param3, param4 in zip(model1.parameters(), model2.parameters(), model3.parameters(), model4.parameters()):
                    ...
                    td1[i] += (param1.grad - gd1[i])
                    td2[i] += (param2.grad - gd2[i]) 
                    td3[i] += (param3.grad - gd3[i]) 
                    td4[i] += (param4.grad - gd4[i])               
                    ...
                    td1[i] += W[0,1]*trd2 + W[0,2]*trd3 + W[0,3]*trd4
                    td2[i] += W[1,0]*trd1 + W[1,2]*trd3 + W[1,3]*trd4
                    td3[i] += W[2,0]*trd1 + W[2,1]*trd2 + W[2,3]*trd4
                    td4[i] += W[3,0]*trd1 + W[3,1]*trd2 + W[3,2]*trd3
...
N = 4                                                    # Number of nodes of the peer-to-peer graph
W = directed(N)                                          # Generate the graph
W = torch.tensor(row_stochastic(W)).float()              # Make it doubly stochastic

# Set the parameters
parser = Object()
parser.folder = '/Code/data/'                            # Set the path
parser.num_shots = 1                                     # Set the number of shots per class
parser.num_testshots = 5
parser.num_ways= 20                                      # Set the number of ways/classes per task
parser.shuffle=True
